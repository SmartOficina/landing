...
@Component({
  ...
  imports: [NgIf, NgFor, NgClass, HeaderComponent, FormsModule, FooterComponent, InputGenericComponent, PaymentComponent, PlansSectionComponent],
})
export class RegistrationComponent implements OnInit, OnDestroy {
  ...
  steps = [{ name: "Cadastro" }, { name: "Pagamento" }];
  ...
  register(): void {
    ...
    this.registrationService.createGarage(registrationData).subscribe({
      next: (response: any) => {
        ...
        if (garageData && garageData._id) {
          ...
          const trialEnabled = environment.trialEnabled;
          const hasPlanSelected = this.selectedPlan && this.selectedPlan._id;

          if (trialEnabled && hasPlanSelected) {
            this.alertService.showAlert("Conta criada com sucesso!", `Parabéns! Você ganhou ${environment.trialDays} dias grátis para testar todas as funcionalidades do plano ${this.selectedPlan.name || "escolhido"}. Aproveite!`, "success", "Acessar Sistema").then(() => {
              if (response.body?.token && this.garage.email) {
                this.authService.garageAuthenticate(this.garage.email, this.garage.password, true).subscribe({
    ...
    if (garageId) {
      this.registrationService.sendActivationEmail(garageId).subscribe({
        ...
        error: (error: any) => {
          ...
          this.alertService.showAlert("Aviso", "Houve um problema ao enviar o e-mail de ativação automaticamente, mas você pode solicitar o reenvio na tela de login.", "warning", "Entendi");
        },
      });
    } else {
      this.isLoading = false;

      this.alertService.showAlert("Aviso", "Não foi possível enviar o e-mail de ativação automaticamente. Você pode solicitar o reenvio na tela de login.", "warning", "Entendi");
    }
  }
